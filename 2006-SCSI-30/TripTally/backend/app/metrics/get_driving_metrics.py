...
def get_all_driving_metrics(distance_km, polyline=None):
    ...
    Returns:
        dict: Driving metrics including fuel_cost_sgd, co2_emissions_kg, erp_charges, and total_cost
    """
    try:
        ...
        co2_emissions = calculate_co2_emissions(distance_km)
        ...
        if polyline:
            try:
                ...
                print(f"Error calculating ERP charges: {str(e)}")
        ...
        print(f"Error in get_all_driving_metrics: {str(e)}")
        raise
...
def get_list_of_passed_gantries(polyline_str):
    ...
    for feature in erp_data:
        if 'coordinates' in feature['info']:
            ...
            if intersects:
                ...
                new_departure_dt = real_datetime + timedelta(seconds=duration_seconds)
                departure_time = new_departure_dt
                
                gantry_id = feature['gantry_no']
                if gantry_id not in gantries_passed:
...
def calculate_erp_charge(gantries_passed):
    ...
    for gantry in gantries_passed:
        ...
        if not weekday_bool or is_time_in_range(gantry[1], "10.01", "17.29"):
            return 0.0  # No ERP charges on weekends/public holidays/between 10am and 5.29pm
        if gantry[0] in erp_rates.keys():
            for time_interval in erp_rates[gantry[0]]['charge_rates']:
                start_time_str, end_time_str = time_interval.split('-')
                if is_time_in_range(gantry[1], start_time_str, end_time_str):
                    total_charge += erp_rates[gantry[0]]['charge_rates'][time_interval]
